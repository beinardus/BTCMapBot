...
async function getGeo(latitude, longitude) {
  try {
    ...
    // public nominatim server uses a fair use policy of one call per second
    const response = await throttle(
      async () => await got(url, constructRequestOptions()), nominatimConfig.throttle);

    const result = response.body.address;

    if (result) 
      return _.pick(result, ["country_code", "country", "state", "county", "municipality", "city", "town", "village"]);
